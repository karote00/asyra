...
test('historical absence of runtime source never triggers new manifest reads or runtime admission', async (t) => {
  ...
  try {
    ...
    delete historical.snapshot.runtimeAuthority
    delete historical.runner.identity.runtimeSourceDigest
    delete historical.runner.identity.runtimeAuthorityFormat
    delete historical.runner.identity.runtimeAuthorityDigest
    delete historical.runner.identity.contractScopeDigest
    delete historical.evidence.runtimeSourceDigest
    delete historical.evidence.runtimeAuthorityFormat
    delete historical.evidence.runtimeAuthorityDigest
    delete historical.evidence.contractScopeDigest
...
test('derived service admission rejects missing authority and actual bytes before dispatch', async () => {
  for (const [label, mutate] of [
    [
      ...
      (snapshot) => Reflect.deleteProperty(snapshot, 'runtimeAuthority')
    ],
    [
  ...
  async () => {
    ...
    try {
      ...
      for (const [label, mutate] of [
        [
          'missing runtime authority',
          (value) => Reflect.deleteProperty(value.snapshot, 'runtimeAuthority')
        ],
        [
          'changed runtime authority',
          (value) => {
            value.snapshot.runtimeAuthority.digest = '0'.repeat(64)
          }
        ],
        [
...
test(
  ...
  async (t) => {
    ...
    try {
      ...
      assert.equal(
        target.targetProof.runtime.runtimeAuthorityDigest,
        f.task.snapshot.runtimeAuthority.digest
      )
      assert.equal(
        target.targetProof.runtime.contractScopeDigest,
        f.task.snapshot.runtimeAuthority.contractScopeDigest
      )
      assert.equal(
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(
        handoff.producers[0].runtimeAuthorityDigest,
        producer.snapshot.runtimeAuthority.digest
      )
      assert.equal(
        handoff.producers[0].contractScopeDigest,
        producer.snapshot.runtimeAuthority.contractScopeDigest
      )
