  ...
  const data = {
    ...
    steps: [
      ...
      },
      {
        ...
        conditions: [
          ...
          'Bound authored trajectory text, format and mapping as schema-valid document input even when conversion or joint-limit validation fails. Executable trajectory validity remains separate; no invalid numeric geometry enters rendering.',
      ...
      },
      {
        ...
        conditions: [
          ...
          'Finite nonnegative interval endpoints persist independently; ordering/coverage restrict execution with immediate field feedback, not unrelated editing. Persist bounded authored trajectory and exclusion inputs through the existing experiment component and editing Feature regardless of executable validity. Independent observation title/text edits permit incomplete content while retaining strict source-reference and capacity guards.',
      ...
      },
      {
        ...
        conditions: [
          'Freeze detached complete inputs and source identities. Reject unresolved authored trajectory/exclusion inputs; composition must consume the storage-owned current valid resolution before preflight, snapshot or Worker allocation.',
      ...
      },
      {
        ...
        conditions: [
          ...
          'Only a completed fresh finite nonpenetrating static warning with 0 < lower <= upper < threshold may seed the next fresh sample inside the same continuous node and ordered immutable mesh pair. The per-pair sampler issues an opaque runtime-branded private-scope handle from its own actual distance inputs/output; continuous/domain shapesAt owns time-to-pose provenance. Admission verifies node, segment, settings, geometry and source time. Frozen geometry and buffers are required. Capture copies/freezes complete interval poses and witness enclosures for one checkpointed unit; consumption charges admission, two inverse transforms, two forward transforms and norm (six units). Capture exhaustion retains the completed sample and stops before further solving without a handle. Whole-enclosure upper seeding retains full target geometry, membership, penetration and all sample/severity decisions; target evidence metadata is recomputed. Native, point-only and unseeded queries retain their ordinary route.',
          'An eligible first fresh source from an unclamped original root may cross exactly one descending adjacent boundary only after the entire root completes as a nonpenetrating finding without exhaustion or subdivision. Publication requires remaining temporal evaluations for the next root and charges one checkpoint; capture is paid once and consumption retains its six charges. Continuous owns completed-root and canonical time/pose provenance; sampler admission retains private scope, frozen ordered geometry, settings, exact adjacent segment/shared boundary and first-fresh-target checks. Only the sampler publication operation converts its own opaque node source into a scoped opaque boundary source sharing the paid immutable payload; an origin flag alone cannot admit an ordinary node source across nodes. Publication exhaustion keeps the completed source leaf and stops without a packet. Consumption follows ordinary evidence-preserving exhaustion. The target runs full geometry, membership, penetration and all original samples; no distance result or metadata is transported.',
        ...
        cacheDimensions: [
          ...
          'Ordinary transported upper-source handles exist between consumed fresh static samples of one current node. The first sample has no same-node seed; only the explicit adjacent-boundary slot may seed an original root start. Inherited, final and ineligible samples, or samples whose next endpoint is inherited, create no handle. The sole exception is one invocation-owned adjacent-boundary source slot from a completed original root to the immediately lower complete original root; it is consumed or discarded at that next root, never retained in a map. Clamped, child, point, native, incomplete and nonadjacent roots cannot use it. No parent-child seed handoff, global registry, arbitrary evidence admission, other cross-node/segment retention, cross-invocation retention or geometry-preparation pose cache is permitted.'
        ],
        implementationBoundary: [
      ...
      },
      {
        ...
        purpose:
          'Produce bounded non-persistent sample feedback without a formal run',
        inputs: [
          'artifact:snapshot',
          'artifact:method-evidence',
          'Feature-owned abort signal',
        ...
        outputs: ['artifact:live-feedback'],
        conditions: [
        ...
        specRefs: ['#live-playback-feedback'],
      ...
      },
      {
        ...
        conditions: [
          ...
          'Own a bounded current document/edit pair of at most two source parses and two conversion receipts per runtime. Reuse exact source text/kind parsing across mapping edits; reuse conversion across editor review, completed edits and execution resolution when text, mapping and joint IDs/kinds/limits are unchanged. Source or validation dependencies invalidate it; unrelated metadata/geometry does not. Resolve stored authored trajectory/exclusion inputs into executable definitions; invalid input returns diagnostics, never the prior trajectory.',
          'Trajectory imports require declared source units. Prepare bounded CSV rows once per source; mapping changes reuse parsing. Domain-admitted converted rows supply both acceptance and bounded first/middle/last source-to-canonical review values, without another conversion. Strict JSON preserves declared units; no preview writes canonical state.',
      ...
      },
      {
        ...
        conditions: [
          ...
          'Setup, Preview and Results are transient keyboard-accessible tabs with candidate/experiment context above them and consistently placed Run/cancel/progress controls near the experiment selector. Context, controls, status, tabs and content share one natural experiment-panel scroll region; no fixed footer or residual-height tab scroll aperture. Keep upper workbench dimensions independent of content. Preserve completed edits, unfinished text and selected evidence without canonical writes. Suspend Preview work on departure. Completion offers View results without switching tabs or stealing focus; ordinary Object inspection stays available.',
          ...
          'Run history and comparison display retained names, experiment revisions, method versions, original candidate/experiment identities and available copied origin. Transient two/three-run slots preserve selection order and allow removal across history pages; changed selections or removed/replaced records retire displayed comparisons. Comparison projects stored evidence with separately labelled execution, coverage and verdict, never executes analysis or rewrites history. Explicit comparison paints pending feedback before computation and prevents duplicate submissions. Selection/source/lifetime changes retire pending work; success scrolls to and focuses the comparison region, while failure restores the action and exposes its error without navigating to stale output. Other explicit project/import/observation/retention actions expose local pending feedback; completed source previews, explicit trajectory previews, observation editor opening, run-detail selection and View results reveal their destination. Background analysis completion and automatic field/save acknowledgements never trigger navigation. Short narrow views stack history and comparison columns with reachable export and keyboard controls.',
    ...
    routes: [
      ...
      },
      {
        ...
        kind: 'normal',
        predicate:
          'The old input surface is retired; Render owns engine resource termination.',
      ...
      },
      {
        ...
        kind: 'normal',
        predicate:
          'Playback has admitted detached inputs and required acknowledgements; a prior formal run is not required.',
        producedArtifacts: ['artifact:snapshot']
      },
      {
        id: 'method-to-live',
        ...
        kind: 'normal',
        predicate:
          'The selected installed method returns evidence for the requested static sample.',
        producedArtifacts: ['artifact:method-evidence']
      },
      {
        id: 'live-to-ui',
        ...
        kind: 'normal',
        predicate:
          'Sample feedback is current for this playback lifetime and includes its checked time.',
        producedArtifacts: ['artifact:live-feedback']
      },
      {
        id: 'live-to-project',
        from: 'live',
      ...
      },
      {
        ...
        kind: 'normal',
        predicate:
          'Validated terminal evidence is presented without changing editing focus; replay consumes its exact frozen inputs and pair records.',
        producedArtifacts: ['artifact:result']
      },
      {
        ...
        kind: 'normal',
        predicate:
          'Immutable run references and storage-session acknowledgement remain distinct; UI presents saving, acknowledged durability or retryable failure.',
    ...
    artifacts: [
      ...
      },
      {
        ...
        channel: 'detached handoff',
        consumerStepIds: ['method', 'run', 'live'],
        terminal: false
      },
      {
