    ...
    steps: [
      ...
      },
      {
        ...
        conditions: [
          ...
          'Completed valid observation text edits apply through the existing editing Feature and keep the editor on the acknowledged observation identity. Incomplete input is transient and stale edits reject. Existing text edits and accepted attachment removal commit independently of pending source preparation without retaining or discarding it. Prepared attachments require Apply attachments before source retention and the separate metadata transaction.',
          'Ordinary experiment field edits apply through the existing editing Feature when the edit gesture completes. Incomplete or invalid field text remains transient. Serialize pending field actions against the latest canonical revision without dropping later edits or replacing them with an older acknowledgement. External Undo/Redo restores the canonical projection. New file import previews remain noncanonical until explicit acceptance.',
          ...
          'App-generated trajectory text starts with known canonical units; new external CSV receives column suggestions with undeclared units. Editing current source text retains existing units and valid mappings without an additional notice or confirmation; missing mapped columns retire only affected declarations, and another CSV requires fresh declarations. Strict JSON keeps its own declarations. The current source-read artifact and storage conversion preview are shared through review and draft acceptance; source, mapping, unit, workcell, cancellation or lifetime changes invalidate acceptance and late reads. UI displays bounded owner-produced conversion values and never converts values itself. Completing an inline trajectory text, mapping or unit edit validates once and merges the valid trajectory with the latest draft through one existing editing Feature, without Apply. Its own canonical acknowledgement preserves raw source text and unit declarations; external canonical replay resets the projection. New file imports require explicit Import trajectory acceptance of the same valid preview. Pending commits prevent duplicate submission. Preview and discard never change canonical state.',
