    ...
    steps: [
      ...
      },
      {
        ...
        conditions: [
          ...
          'Unchanged App-generated trajectory text starts with known canonical units; new external CSV receives column suggestions with undeclared units. First edits retain displayed canonical choices with a one-action unit confirmation notice; those choices are not declarations and cannot authorize acceptance until explicitly confirmed. Explicit user unit declarations survive numeric edits of the same source; missing mapped columns retire only affected declarations, and another CSV requires fresh declarations. Strict JSON keeps its own declarations. The current source-read artifact and storage conversion preview are shared through review and draft acceptance; source, mapping, unit, workcell, cancellation or lifetime changes invalidate acceptance and late reads. UI displays bounded owner-produced conversion values and never converts values itself or changes canonical state before the existing save Feature.',
