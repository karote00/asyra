...
import { priorityItemField } from '../../examples/priority/priority-item-field.js'
...
describe('default App priority onboarding', () => {
  ...
  it('preserves a pending Redo after rejecting an invalid saved priority', async () => {
    const storage = new MemoryStorage()
    ...
    try {
      ...
      const id = runtime.feature.addItem({ title: 'Redo item' })
      runtime.feature.editItem(id, { fields: { priority: 'high' } })
      await runtime.undo()
      await settleProjection()
      expect(priorityOf(runtime, id)).toBe('normal')

      const beforeCore = await runtime.core.save()
      const beforeProjection = runtime.projection.getSnapshot()
      const beforeDepth = runtime.core.getUndoHistoryDepth()
      const invalidCore = structuredClone(beforeCore)
      itemProperty(invalidCore).priority = 'urgent'
      storage.setItem(
        STARTER_STORAGE_SLOT,
        JSON.stringify(createStarterDocumentWrapper(invalidCore))
      ...
      expect(await runtime.core.save()).toEqual(beforeCore)
      expect(runtime.projection.getSnapshot()).toEqual(beforeProjection)
      expect(runtime.core.getUndoHistoryDepth()).toBe(beforeDepth)
      await runtime.redo()
      await settleProjection()
      expect(priorityOf(runtime, id)).toBe('high')
    } finally {
      await runtime.dispose()
    }
  })
