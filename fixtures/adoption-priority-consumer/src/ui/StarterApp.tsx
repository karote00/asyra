...
import { ITEM_PRIORITIES } from '../examples/priority/priority-item-field.js'
import {
...
const SelectedItemEditor = ({
  ...
  return (
    <div className="item-editor">
      ...
      <div className="status-control" role="group" aria-label="Priority">
        {ITEM_PRIORITIES.map((priority) => (
          <button
            key={priority}
            type="button"
            aria-pressed={priority === item.fields?.priority}
            className={
              priority === item.fields?.priority ? 'active' : undefined
            }
            onClick={() => {
              if (priority === item.fields?.priority) return
              try {
                runtime.feature.editItem(item.id, { fields: { priority } })
                onEdit('Updated priority - unsaved changes')
              } catch (error) {
                onEdit(errorMessage(error))
              }
            }}
