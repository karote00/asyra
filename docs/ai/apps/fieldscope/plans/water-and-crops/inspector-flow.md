...
- Cache dimensions: runtime-owned admitted crop geometry uses netTop/netBottom, retains at most two entries, and at most 32 primitive shapes keyed by tube displacement/diameter, clip axes/normal/diameters or tie radius; plus the latest envelope/terrain/cultivation/crop projections keyed by their scalar configuration dependencies, and grouped crop instances keyed by width/strip kind/strip width, length, soilInset, startInset and endInset, all cleared on runtime disposal. Per-mesh detail matrices remain valid only for matching camera projection/view, viewport height, world transform, admitted instances, geometry and maxError; runtime-local weak bounds retain immutable shape extrema, while each measurement reduces current instance transforms; mutable shapes bypass retention. GPU surface maps are shared by admitted surface identity while materials retain them, and released with the last material.
