...
Touch navigation uses one finger for stationary look, two fingers for camera-plane pan, and two-finger separation for dolly (spread to approach, pinch to retreat). Dolly uses the same distance/zoom bounds as the wheel; a separation ratio of two doubles displayed zoom. Finger addition/removal does not itself move the camera. Three or more fingers suspend navigation, retaining current positions for a continuous handoff. Pointer release/cancellation/capture loss removes that pointer; window blur and teardown clear all pointers. The canvas owns touch gestures while panels retain normal page scrolling. These view operations never write configuration history.
