...
- Canonical configuration lives in Core Props. Unfinished field text, layer visibility, panel state, and camera are transient presentation state.
- Each completed field edit or discrete strip action is one transaction. Undo and Redo use Core history. Reload starts from defaults; there is no saved-project persistence yet.
