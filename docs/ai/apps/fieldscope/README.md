...
相機與 target 同步平移，與光學 zoom 分開。基準速度為目前預設視角的垂直視野跨度
`2 × referenceDistance × tan(referenceFOV / 2)` 的 30%／秒。
實際速度為 `基準速度 × max(0.3, sqrt(100 / zoomPercent))`，Shift 再乘 4。
400% 為基準速度的 1/2；高倍率最低保留 30%，避免光學放大後無法快速進入溫室。
預設透視 100% 約 18m/s，1521% 約 5.4m/s；夾具近看使用自身公分尺度的基準速度，
不套用全景的公尺速度下限。切換預設視角與場景尺寸變更會更新基準，fit 透過 zoom 比例反映。
按鍵端傳入每秒 1.5 個方向單位；相機端統一換算距離。單次按下使用 1/30 秒位移，
按住依幀間時間連續移動，多軸方向正規化。
只在持續按鍵期間排程 animation frame；放開、焦點離開、視窗失焦、頁面隱藏及卸載時停止。
右鍵拖曳固定相機位置原地轉頭，以世界向上方向避免側滾；左鍵繞 target 旋轉、
Shift 左鍵 pan、滾輪 zoom、方向鍵旋轉、+/- zoom、⌘1/⌘0 與設定 Undo/Redo 保留。
