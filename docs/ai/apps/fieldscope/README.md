...
相機與 target 同步平移，與光學 zoom 分開。移動速度依目前 target 距離與 FOV 決定：
每秒移動基準垂直視野跨度 `2 × distance × tan(FOV / 2)` 的 30%，Shift 加速為 4 倍。
相同視角放大 10 倍，移動距離縮小為 1/10；fit、切換視角與場景尺寸變更也自動反映。
預設透視 100% 約 18m/s，可快速進入溫室；夾具近看則依細節尺度微移。
按鍵端傳入每秒 1.5 個方向單位，相機端每單位換算為視野跨度的 1/5；
單次按下前進視野跨度的 1%，按住則依幀間時間連續移動，多軸方向正規化。
