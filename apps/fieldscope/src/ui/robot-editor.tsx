...
function RobotEditor({ runtime }: { runtime: FarmRuntime }) {
  ...
  const walkingSelection = useSyncExternalStore(
    runtime.subscribeWalkingOperatingReport,
    runtime.getWalkingOperatingReport
  ).selection
  ...
  return (
    ...
    >
      ...
      <WalkingRuntimeSelector runtime={runtime} selection={walkingSelection} />
      <button
        type="button"
      ...
      {walkingSelection.mode === 'legacy-view' ? (
        <>
          ...
          <details className="rounded-lg border border-[#d9dfd2] bg-white p-3 text-xs">
            ...
            {report.lane &&
              [...report.lane.blocked, ...report.lane.unverified].map(
                (reason) => (
          ...
          <div className="space-y-2">
            <label className="flex items-center justify-between gap-2 text-xs">
              ...
              <select
                ...
                onChange={(e) =>
                  void patch({
                    tool: e.target.value as RobotConfiguration['tool']
                  })
                }
              >
            ...
            <label className="flex items-center justify-between gap-2 text-xs">
              ...
              <select
                ...
                value={laneId}
                onChange={(e) => {
                  const selected = lanes.find(
                    (lane) => lane.id === e.target.value
                  )
        ...
        <p className="rounded-lg bg-[#edf2e5] p-3 text-xs leading-relaxed text-[#50664f]">
          {t('robot.model.legacyControlsUnavailable')}
        </p>
      )}
    </section>
  )
