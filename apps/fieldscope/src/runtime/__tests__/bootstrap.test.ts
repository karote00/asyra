...
it('keeps one geometry construction through navigation and queued display changes, then retires it', async () => {
  ...
  try {
    ...
    expect.soft(movementDistance()).toBeCloseTo(overviewSpeed / 2, 8)
    runtime.actualSize()
    ...
    expect.soft(doorwaySpeed).toBeCloseTo(overviewSpeed * 0.3, 8)
    runtime.zoom(-10000)
    const detailSpeed = movementDistance()
    expect.soft(detailSpeed).toBeCloseTo(overviewSpeed * 0.3, 8)
    runtime.actualSize()
    expect.soft(movementDistance()).toBeCloseTo(overviewSpeed, 8)
    await runtime.setCamera('joint')
    const jointSpeed = movementDistance()
    expect.soft(jointSpeed).toBeLessThan(0.1)
    runtime.zoom(-10000)
    expect.soft(movementDistance()).toBeCloseTo(jointSpeed * 0.3, 8)
    await runtime.setCamera('overview')
    notify.mockClear()
