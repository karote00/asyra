...
it('spans 24 longitudinal rows from 45cm to transverse beam height with a tie at every pole', () => {
  const assembly = createSupportAssembly()
  const net = createPlantingNet(assembly)
  expect(net.ties).toHaveLength(1992)
  expect(net.strands).toHaveLength(24 * (329 + 18))
  ...
  for (const strand of net.strands) {
    ...
    if (strand.points[0][2] === strand.points[1][2]) {
      ...
      expect(strand.points[1][1]).toBeCloseTo(3)
    }
  }
  net.ties.forEach((tie, index) => {
    ...
    expect(tie.center[1] + 0.002).toBeCloseTo(3)
    expect(tube.points[1][1] - (tie.center[1] + 0.002)).toBeCloseTo(0.15)
