...
export function createDefaultExperimentDraft(
  ...
  const primaryBodyIds = workcell.bodies
    .filter((body) => body.role !== 'fixture')
    .map((body) => body.id)

  const influencingBodyIds = workcell.bodies
    .filter((body) => body.role === 'fixture')
    .map((body) => body.id)
...
export function guessCsvMapping(
  ...
  const take = (preferred: (column: string) => boolean): string => {
