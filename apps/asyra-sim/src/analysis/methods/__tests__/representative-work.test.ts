...
it(
  'completes the unchanged representative workcell with exact paid geometry work',
  async () => {
    progress?.stage('representative-fixture')
    ...
    const evidence = runOriginalPartMethod(
      snapshot,
      progress?.query('representative-query', () => activeWork()),
      (pair) => {
        ...
        const sourceWitnessWork =
          counts.sourceWitnessWork - previousSourceWitness
    ...
    progress?.endQuery()
    progress?.stage('representative-assertions')
    ...
    expect(
      ...
      [...observedContexts].reduce((sum, context) => sum + context.work, 0)
    )
