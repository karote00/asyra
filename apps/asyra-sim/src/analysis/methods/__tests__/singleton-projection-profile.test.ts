...
describe.runIf(process.env.SIM_CAPACITY_DIAGNOSTICS === '1')(
  'fixed singleton projection population',
  () => {
    it.each(['control', 'candidate'] as const)(
      ...
      async (mode) => {
        ...
        type Project = (
          a: ConvexShape,
          b: ConvexShape,
          an: mesh.MeshNode | undefined,
          bn: mesh.MeshNode | undefined,
          gap: number,
          threshold: number
        ) => number
        ...
        vi.spyOn(owner, 'nodeGap').mockImplementation(function (
          ...
          const original = () =>
            nodeGap.call(this, a, b, an, bn, gap, threshold)
          if (mode === 'control') return original()
          let delegated = false
          const result = terminalNodeGap(an, bn, gap, () => {
            delegated = true
            return original()
          })
          if (!delegated) calls.omittedStaticProjection++
          return result
        })
        ...
        const artifactPath = fileURLToPath(
          new URL(
            `../../../../../../tmp/capacity/singleton-projection-${mode}.json`,
            import.meta.url
          )
        )
        ...
        console.info(
          JSON.stringify({
            profile: 'singleton-projection-policy',
            mode,
            work: context.work,
        ...
        if (mode === 'control') {
          ...
          expect(nested.staticAxes).toBeGreaterThan(0)
