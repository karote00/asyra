...
import { partitionIndex } from './sah-index-fixture'
...
describe.runIf(process.env.SIM_CAPACITY_DIAGNOSTICS === '1')(
  ...
  () => {
    ...
    it.each([2, 4])(
      'measures fixed source-complete SAH preparation over second %s',
      async (start) => {
        ...
        vi.spyOn(meshIndex, 'buildMeshIndex').mockImplementation(
          (mesh, checkpoint, hierarchy) => {
            ...
            const result = partitionIndex(index, () => {
              addedPreparation++
              checkpoint()
            })
        ...
        console.info(
          JSON.stringify({
            profile: 'fixed-eight-bin-source-sah',
            start,
    ...
    it('measures the fixed SAH preparation against the unchanged full representative budget', async () => {
      const snapshot = await representativeSnapshot(0)
      let originalPreparation = 0,
        addedPreparation = 0,
        builds = 0
      ...
      console.info(
        JSON.stringify({
          profile: 'full-fixed-eight-bin-source-sah',
          builds,
          originalPreparation,
          addedPreparation,
