...
export const ORIGINAL_PART_METHOD: InstalledMethodDescriptor = {
  ...
  manifest: {
    ...
    reproducibility:
      'Deterministic source-order geometry and median hierarchy. After root rejection and solid-membership admission, first actual traversal prepares complete admitted-component median subtrees and a median component top tree, preserving all source triangles and charging original plus demanded refinement preparation equally on cold and warm use; dual-tree descent splits the larger maximum world-bound extent, left on ties. Each temporal leaf tests its first static witness and then attempts a full-interval clearance certificate before further static sampling; unsuccessful attempts retain their work charge. Completed endpoint evidence passes only from parent to children in the same original segment; a completed same-node mesh interval certificate passes to final certification only with positive first and final witness admission. Each handoff charges one work unit and checks cancellation. At the existing certificate request, an in-node completed source witness at or below threshold derives the exact mesh threshold-certificate zero with one charged checkpoint; all remaining static samples and severity decisions remain. Existing completed-certificate handoff has priority, and native axis-sensitive lower queries are recomputed. Complete source bounds use inverse world-axis projections. Root and hierarchy-node bounds then use fixed midpoint pose-axis rejection in source order with cardinal repeats omitted and each attempted axis charged. Final original triangle pairs use world-gap rejection then the complete convex query, retaining their existing triangle work charge without extra pose-axis projection. Fixed ray directions with enclosed predicates. No random seed. Chromium arithmetic conformance required.',
    resources:
